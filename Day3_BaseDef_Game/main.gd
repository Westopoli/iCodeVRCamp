...
# TODO #1: Give the game the four pieces of state it remembers — two empty lists and two counters.
#
# Syntax:
...
#
# Given:
#   - START_COINS      — constant at the top of the file (starting coin amount)
#   - START_BASE_HP    — constant at the top of the file (starting base health)
#
# Line by line:
...
func _process(delta: float) -> void:
	...
	# TODO #3: Move the whole world this frame — step every enemy, then tick every tower.
	#
	# Syntax:
	#   - for item in list:
	#
	# Given:
	#   - enemies            — list of all active enemy nodes
	#   - towers             — list of all placed tower nodes
	#   - step_enemy(e, delta)   — pre-given: moves one enemy forward one step
	#   - tower_tick(t, delta)   — pre-given: lets one tower scan for targets and fire
	#
	# Line by line:
	...
	# TODO #7: When a wave is finished, advance to the next wave — or win the game if that was the last one.
	#
	# Syntax:
	#   - if cond and cond:
	...
	#
	# Given:
	#   - wave_in_progress         — true while a wave is running
	#   - enemies                  — list of active enemies (empty = all dead)
	#   - enemies_to_spawn         — queue of enemies still waiting to appear
	#   - wave_index               — which wave number we're on (0-based)
	#   - WAVES                    — the full list of waves
	#   - you_win()                — pre-given: triggers the win screen
	#   - start_next_wave()        — pre-given: loads the next wave
	#
	# Line by line:
...
func spawn_enemy(spawn_cell: Vector2i, enemy_type: String) -> void:
	...
	# TODO #2a: Put this freshly built enemy onto the enemies list so the game loop can see it.
	#
	# Syntax:
	#   - list.append(item)
	#
	# Given:
	#   - e          — the enemy node just built above this hole
	#   - enemies    — the list of all active enemies
	#
	# Line by line:
...
func kill_enemy(e: Node, give_reward: bool = true) -> void:
	...
	if give_reward:
		...
		# TODO #2b: A tower killed this enemy — take it off the list and pay the player the reward.
		#
		# Syntax:
		#   - list.erase(item)
		#   - counter += amount
		#
		# Given:
		#   - e         — the enemy node to remove
		#   - enemies   — the list of active enemies
		#   - reward    — the coin payout amount (unpacked above this hole)
		#   - coins     — the player's current coin total
		#
		# Line by line:
		...
		# TODO #2b: This enemy reached the base — take it off the list, but pay no reward (no tower killed it).
		#
		# Syntax:
		#   - list.erase(item)
		#
...
func move_all(enemy_list: Array, delta: float) -> void:
	# TODO #4: Fill in move_all so it steps every enemy in the list it was handed (enemy_list, not enemies).
	#
	# Syntax:
	#   - for item in list:
	...
	#
	# Given:
	#   - enemy_list         — the list parameter handed to this function (not the global `enemies`)
	#   - step_enemy(e, delta)   — pre-given: moves one enemy forward one step
	#
	# Line by line:
...
func get_nearest_enemy_in_range(pos: Vector2, tower_range: float) -> Node:
	...
	# TODO #5a: Scan the enemies and find the closest one that's still inside the tower's range.
	#
	# Syntax:
	#   - for item in list:
	#   - if cond and cond:
	...
	#   - pos                  — the tower's world position (parameter)
	#   - tower_range          — the tower's range radius (parameter)
	#   - nearest              — starts as null; updated when a closer enemy is found
	#   - best_dist            — starts just outside range; updated each time nearest updates
	#   - pos.distance_to(e.position)  — built-in: returns distance between two positions
	#
	# Line by line:
...
func get_enemies_in_radius(pos: Vector2, radius: float) -> Array:
	# TODO #5b: Build and return a brand-new list holding every enemy inside the blast radius.
	#
	# Syntax:
	#   - var name: Array = []    (empty list)
	#   - for item in list:
	...
	#   - enemies              — list of all active enemies
	#   - pos                  — the tower's world position (parameter)
	#   - radius               — the splash radius (parameter)
	#   - pos.distance_to(e.position)  — built-in: returns distance between two positions
	#
	# Line by line:
...
func tower_tick(t: Node, delta: float) -> void:
	...
	# TODO #6 lives in the two branches below. Pre-given: this match dispatches
	# by tower type — Cannon/Sniper share a single-target branch (#6a), Splash
	# gets its own list-target branch (#6b). Kid fills each branch body.
	match t_type:
		"cannon", "sniper":
			# TODO #6a: Cannon and Sniper find one target, then fire at it and reset the cooldown.
			#
			# Syntax:
			...
			# Given:
			#   - t                                     — the tower node
			#   - t.position                            — the tower's world position
			#   - t_range, t_damage, t_rate             — unpacked above this hole
			#   - get_nearest_enemy_in_range(pos, range) — returns one enemy or null
			#   - fire_at(tower, target, damage)        — pre-given: fires and deals damage
			#   - t.set_meta("cooldown", t_rate)        — resets the cooldown timer
			#
			# Line by line:
			...
			#@end
		"splash":
			# TODO #6b: Splash collects every target in its radius, then fires at the whole list and resets the cooldown.
			#
			# Syntax:
			#   - var x = func_name(args)
			#   - if list.size() > 0:
			...
			#   - t.position                            — the tower's world position
			#   - t_range, t_damage, t_rate             — unpacked above this hole
			#   - get_enemies_in_radius(pos, radius)    — returns a list of enemies in range
			#   - fire_at(tower, targets, damage)       — pre-given: fires at a list of enemies
			#   - t.set_meta("cooldown", t_rate)        — resets the cooldown timer
			#
			# Line by line:
