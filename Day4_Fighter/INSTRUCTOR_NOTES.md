...
1. Open Godot 4.6.x.
2. Import `Day4_Fighter/project.godot`.
...
```powershell
.\build\build_templates.ps1 -Day Day4_Fighter
```

Generates `dist/Day4_Fighter_Template.zip` (kid scaffold, won't compile until chunks filled) + `dist/Day4_Fighter_Complete.zip` (instructor reference, source of truth).
