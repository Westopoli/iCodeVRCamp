...
# --- Pre-given: the 4 core game-state variables ---
var enemies: Array = []
var towers: Array = []
...
func _process(delta: float) -> void:
	...
	# TODO #4   Run the world each frame by CALLING the two functions you wrote.
	# You already defined move_all() and tick_all_towers() below. Now call them
	# here so they actually run every frame:
	#     - call move_all with the enemies list and delta
	#     - call tick_all_towers with delta
	#
	# Given:
	#   - move_all(enemy_list, delta)   — the function you wrote (TODO #1)
	#   - tick_all_towers(delta)        — the function you wrote (TODO #2)
	...
	#@todo
	move_all(enemies, delta)
	tick_all_towers(delta)
	#@end

	# --- Pre-given: when the wave is cleared, advance to the next one (or win) ---
...
func spawn_enemy(spawn_cell: Vector2i, enemy_type: String) -> void:
	...
	# --- Pre-given: add this enemy to the list so the game loop sees it ---
	enemies.append(e)
...
func kill_enemy(e: Node, give_reward: bool = true) -> void:
	...
	if give_reward:
		...
		# --- Pre-given: remove the dead enemy and pay the bounty. The payout
		# goes through reward_coins() — the function you write in TODO #3. ---
		enemies.erase(e)
		reward_coins(reward)
	else:
		# --- Pre-given: enemy reached the base — remove it, no coins ---
		enemies.erase(e)
	e.queue_free()


func move_all(enemy_list: Array, delta: float) -> void:
	# TODO #1   Fill in this FUNCTION's body.
	# move_all takes a list of enemies (the parameter `enemy_list`) and a time
	# value (`delta`). Make it move every enemy one step: go through enemy_list
	# and call step_enemy(e, delta) on each one.
	#
	# Given:
...
func tick_all_towers(delta: float) -> void:
	# TODO #2   Fill in this FUNCTION's body.
	# tick_all_towers takes a time value (`delta`). Make it run every tower one
	# step: go through the towers list and call tower_tick(t, delta) on each one.
	# (This is the same shape as move_all — a function that loops a list.)
	#
	# Given:
	#   - towers                 — the list of all placed towers
	#   - tower_tick(t, delta)   — runs one tower's targeting + firing
	#
	# Syntax:
	#   - for t in towers:
	#         do_something(t)
...
func reward_coins(amount: int) -> void:
	# TODO #3   Fill in this FUNCTION's body.
	# reward_coins takes a number (the parameter `amount`). Make it pay the
	# player by adding `amount` to the coins variable. One line.
	#
	# Given:
	#   - amount   — the number of coins to add (passed in to this function)
	#   - coins    — the player's coin counter
	#
	# Syntax:
	#   - coins += amount      (adds amount to coins)
	#@todo
	coins += amount
	#@end
...
func get_nearest_enemy_in_range(pos: Vector2, tower_range: float) -> Node:
	...
	# --- Pre-given: scan the enemies and keep the closest one in range ---
	for e in enemies:
...
func get_enemies_in_radius(pos: Vector2, radius: float) -> Array:
	# --- Pre-given: collect every enemy inside the blast radius and return them ---
	var result: Array = []
	for e in enemies:
...
func tower_tick(t: Node, delta: float) -> void:
	...
	# --- Pre-given: pick a target for this tower type and fire at it. This calls
	# the finder + fire helpers for you — it's the same kind of function-calling
	# you practiced in TODO #1-#4, just done for you here. ---
	match t_type:
		"cannon", "sniper":
