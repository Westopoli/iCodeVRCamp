...
#
#  Today's tool: LOOPS (for, while). You write the loops; the
#  functions they live in are already set up for you.
#
...
func _ready():
	...
	# TODO #1: Spawn 3 ghosts. Write a loop that runs 3 times and calls
	# spawn_ghost_at(ghost_spawn_pos(i)) each time.
	# Syntax:
	#   for i in range(3):
	#@todo
	for i in range(3):
	...
	# --- Pre-given: store the dot total so the win check has a target. ---
...
func _process(delta):
	...
	if PERSONALITY_MODE_ENABLED:
		...
		# TODO #2: Step every ghost. Write a loop over the ghosts list that
		# calls step_ghost(ghost) on each one.
		# Syntax:
		#   for ghost in ghosts:
		#@todo
		for ghost in ghosts:
...
# ============================================================
#  HELPER FUNCTIONS — pre-given. Read them if you want, but you
#  don't need to change anything here. Your loops are the TODOs.
# ============================================================

# --- Pre-given: send the player back to the starting tile with no
# leftover movement. ---
...
# --- Pre-given: move the player one tile in direction, but only if no
# wall is there (and wrap through tunnels). ---
...
# --- Pre-given: hit_wall(cell) -> bool. True if that cell is a wall
# (or off-grid), false if open. Tunnel rows let the player wrap. ---
func hit_wall(cell: Vector2i) -> bool:
	...
	if cell.x < 0 or cell.x >= MAZE_W:
		if cell.y in TUNNEL_ROWS:
			return false
		return true
	# Pre-given: inside the maze, a tile present (source id != -1) is a wall.
	var source_id := wall_layer.get_cell_source_id(cell)
	return source_id != -1


# Pre-given function header + var count + return. You write ONLY the loop.
func count_dots() -> int:
	var count := 0   # pre-given: tally starts at zero
	# TODO #3: Tally the dots. Write nested loops that walk every column x
	# (0..MAZE_W) and every row y (0..MAZE_H), and do count += 1 whenever
	# cell_has_dot(x, y) is true.
	# Syntax:
	#   var x := 0
	#   while x < MAZE_W:
	#       ... (inner while y < MAZE_H loop) ...
	#       x += 1
	#@todo
	var x := 0
	while x < MAZE_W:
		...
		x += 1
	#@end
	return count   # pre-given: hand back the final tally
